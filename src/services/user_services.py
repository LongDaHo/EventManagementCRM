...
from models.dynamo_items import UserPersonalInfo, UserProfessionalInfo, UserEventCount
...
async def get_user_based_on_query(queries: List[UserQueryInput]):
    ...
    for query in queries:
        if query.company:
            response = await get_user_based_on_company(query.company, query.limit, query.last_evaluated_key, query.ascending)
        elif query.jobTitle:
            response = await get_user_based_on_job_title(query.jobTitle, query.limit, query.last_evaluated_key, query.ascending)
        elif query.city and query.state:
            response = await get_user_based_on_city_state(query.city, query.state, query.limit, query.last_evaluated_key, query.ascending)
        elif query.minEventAttendedCount and query.maxEventAttendedCount:
            response = await get_user_based_on_attended_events(query.minEventAttendedCount, query.maxEventAttendedCount, query.limit, query.last_evaluated_key, query.ascending)
        elif query.minEventHostCount and query.maxEventHostCount:
            response = await get_user_based_on_hosted_events(query.minEventHostCount, query.maxEventHostCount, query.limit, query.last_evaluated_key, query.ascending)
        users = []
...
async def get_user_based_on_company(company: str, limit: int, last_evaluated_key: dict, ascending: bool):
...
async def get_user_based_on_job_title(job_title: str, limit: int, last_evaluated_key: dict, ascending: bool):
...
async def get_user_based_on_city_state(city: str, state: str, limit: int, last_evaluated_key: dict, ascending: bool):
...
async def get_user_based_on_hosted_events(min_hosted_events: int, max_hosted_events: int, limit: int, last_evaluated_key: dict, ascending: bool):
    logger.info(f"Getting users based on hosted events: {min_hosted_events} to {max_hosted_events}")
    response = UserEventCount.query_item(
        index_name='GSI_HostedEvents',
        key_condition_expression=Key('hostedEvents').between(min_hosted_events, max_hosted_events),
        limit=limit,
        last_evaluated_key=last_evaluated_key,
        ascending=ascending
    )
...
async def get_user_based_on_attended_events(min_attended_events: int, max_attended_events: int, limit: int, last_evaluated_key: dict, ascending: bool):
    logger.info(f"Getting users based on attended events: {min_attended_events} to {max_attended_events}")
    response = UserEventCount.query_item(
        index_name='GSI_AttendedEvents',
        key_condition_expression=Key('attendedEvents').between(min_attended_events, max_attended_events),
        limit=limit,
        last_evaluated_key=last_evaluated_key,
        ascending=ascending
    )
