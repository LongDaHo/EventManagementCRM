...
class BaseQueryInput(BaseModel):
    ...
    def check_attended_event_count(cls, data):
        ...
        if (minEventAttendedCount and not maxEventAttendedCount) or (maxEventAttendedCount and not minEventAttendedCount):
            raise ValueError("Both 'minEventAttendedCount' and 'maxEventAttendedCount' must be provided together.")
        if (minEventAttendedCount and maxEventAttendedCount) and (minEventAttendedCount > maxEventAttendedCount):
            raise ValueError("minEventAttendedCount must be less than maxEventAttendedCount.")
        return data
    ...
    def check_hosted_event_count(cls, data):
        ...
        if (minEventHostCount and not maxEventHostCount) or (maxEventHostCount and not minEventHostCount):
            raise ValueError("Both 'minEventHostCount' and 'maxEventHostCount' must be provided together.")
        if (minEventHostCount and maxEventHostCount) and (minEventHostCount > maxEventHostCount):
    ...
    def check_only_one_filter_group(cls, data):
        ...
        if data.minEventAttendedCount is not None and data.maxEventAttendedCount is not None:
        ...
        if len(filters) == 0:
            raise ValueError("You must provide exactly one filter group (e.g., company, jobTitle, city+state, etc.)")
        if len(filters) > 1:
            raise ValueError(f"Only one filter group is allowed per query. You provided: {filters}")
        return data
